...
AddEventHandler('outlawNotify', function(alert)
		if PlayerData.job ~= nil and PlayerData.job.name == 'bac' then
            Notify(alert)
        end
end)
...
Citizen.CreateThread( function()
    while true do
        ...
        if IsPedTryingToEnterALockedVehicle(GetPlayerPed(-1)) or IsPedJacking(GetPlayerPed(-1)) then
			...
			if PlayerData.job ~= nil and PlayerData.job.name == 'bac' and showcopsmisbehave == false then
			elseif PlayerData.job ~= nil and PlayerData.job.name == 'bac' and showcopsmisbehave then
						...
						ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
							...
							if s2 == 0 then
								if IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
									TriggerServerEvent('thiefInProgressS1bac', street1, vehName2, sex)
								else
